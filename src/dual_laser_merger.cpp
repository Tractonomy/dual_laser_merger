...
{
MergerNode::MergerNode(const rclcpp::NodeOptions& options) : Node("dual_laser_merger", options)
{
  declare_param();
...
void MergerNode::declare_param()
{
  ...
  input_queue_size_param = this->declare_parameter("queue_size", static_cast<int>(std::thread::hardware_concurrency()));
  ...
  else
  {
    if (enable_average_filter_param)
    {
      ...
      for (size_t i = 0; i <= lidar_1_msg->ranges.size(); i++)
      {
        if (i == 0)
        {
          lidar_1_avg.ranges[i] = (lidar_1_msg->ranges[lidar_1_msg->ranges.size() - 1] + lidar_1_msg->ranges[i] +
                                   lidar_1_msg->ranges[i + 1]) /
                                  3;
        }
        ...
        {
          lidar_1_avg.ranges[i] =
              (lidar_1_msg->ranges[i - 1] + lidar_1_msg->ranges[i] + lidar_1_msg->ranges[i + 1]) / 3;
        }
      }
      for (size_t i = 0; i <= lidar_2_msg->ranges.size(); i++)
      {
        if (i == 0)
        {
          lidar_2_avg.ranges[i] = (lidar_2_msg->ranges[lidar_2_msg->ranges.size() - 1] + lidar_2_msg->ranges[i] +
                                   lidar_2_msg->ranges[i + 1]) /
                                  3;
        }
        ...
        {
          lidar_2_avg.ranges[i] =
              (lidar_2_msg->ranges[i - 1] + lidar_2_msg->ranges[i] + lidar_2_msg->ranges[i + 1]) / 3;
        }
      }
